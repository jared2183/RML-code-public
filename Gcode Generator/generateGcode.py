...
class NetworkGcodeGenerator: 
    ...
    def generate_network_gcode(self, inpath, outpath, view=False): 
        ...
        for index, row in edge_df.iterrows():
            material_index = int(row['stimulus'])  # should i catch if this column doesnt exist? for single material prints

            node1_position = node_df.iloc[int(row['EndNodes_1']) - 1]    # subtract 1 because node numbers start at 1 in the excel file and dataframes are zero-indexed
            node2_position = node_df.iloc[int(row['EndNodes_2']) - 1]

            print_speed_mmps = row['print_speed_mmps']
            print_pressure_psi = row['print_pressure_psi']
            
            xy_spacing_mm = row['xy_spacing_mm']
            numpaths_xy = int(row['numpaths_xy'])

            firstlayerheight_mm = row['firstlayerheight_mm']
            z_layerheight_mm = row['z_layerheight_mm']
            numlayers_z = int(row['numlayers_z'])
            
            # adds gcode to print the connection between the two nodes
            g.print_connection(material_index=material_index,
                               x0=node1_position['x'], y0=node1_position['y'], x1=node2_position['x'], y1=node2_position['y'], 
                               print_speed_mmps=print_speed_mmps, print_pressure_psi=print_pressure_psi, 
                               numpaths_xy=numpaths_xy, xy_spacing_mm=xy_spacing_mm, 
                               firstlayerheight_mm= firstlayerheight_mm, z_layerheight_mm=z_layerheight_mm, numlayers_z=numlayers_z)
    ...
    def generate_all(self, view=False):
        ...
        for inpath in sorted(input_folder.glob('*.xlsx')):
            ...
            outpath = output_folder / f"{inpath.stem}.pgm"
